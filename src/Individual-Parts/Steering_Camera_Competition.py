...
class GyroController:
    def __init__(self):
        ...
        self.servo = AngularServo(
            ...
            min_pulse_width=0.0011,
            max_pulse_width=0.0018
        )
    ...
    try:
        while True:
            ...
            if button.is_pressed and not mission_complete:
                ...
                # Continue steering while running (regardless of button state now)
                if gyro.running:
                ...
                # ONLY continue running if already started, ignore button state during mission
                frame = picam2.capture_array()
                frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
                ...
                # Not running and button not pressed
                # If button released after mission complete, allow restart
