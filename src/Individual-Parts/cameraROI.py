...
COLOR_LOW  = (0, 0, 0)   # lower HSV bound
COLOR_HIGH = (50, 255, 50)  # upper HSV bound
...
def gen_frames():
    ...
    while True:
        ...
        difference = left_pixels - right_pixels   # + = steer right, - = steer left

        k = 0.05                                   # proportional gain
        target = difference * k                   # angle in degrees

        # clamp to [-90, +90]
        target = max(-90, min(90, target))

        # Draw on screen
        cv2.putText(frame, f"target: {target:.1f}",
                    (100, h - 20), cv2.FONT_HERSHEY_SIMPLEX,
                    0.8, (255, 255, 255), 2)
