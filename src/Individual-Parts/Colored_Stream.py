...
AVOIDANCE_WEIGHT = 0.003  # Degrees per pixel of obstacle detected
AVOIDANCE_THRESHOLD = 100  # Minimum pixels to trigger avoidance
MAX_AVOIDANCE = 3.0  # Max degrees per frame from avoidance alone
...
class VisionSteeringController:
    def __init__(self):
        ...
        self.avoidance_smooth_window = 3  # Frames to average over
        
    def update(self, left_pixels, right_pixels, avoidance_bias=0.0):
        """
        Update target angle based on wall pixel difference + obstacle avoidance
        avoidance_bias: positive = steer right, negative = steer left
        """
        with self.lock:
            ...
            # Smooth avoidance bias
            smoothed_avoidance = self.smooth_avoidance(avoidance_bias)
            
            # Add avoidance bias to rate
            # Allow avoidance to slightly exceed max_rate for safety
            total_rate = rate + smoothed_avoidance
            total_rate = max(-self.max_rate * 1.5, min(self.max_rate * 1.5, total_rate))
    ...
    def should_stop(self, current_rotation):
        ...
        with self.lock:
            # Check if active and if GYRO cumulative rotation is in range
            in_range = self.lap_min <= abs(current_rotation) <= self.lap_max
            if self.is_active and in_range:
                print(f"[Auto-Stop] Gyro rotation {current_rotation:.1f}° is in range [{self.lap_min:.0f}°, {self.lap_max:.0f}°]")
            return self.is_active and in_range
    
    def start(self):
        """Activate vision steering"""
        with self.lock:
            self.is_active = True
            self.avoidance_history = []  # Clear history on start
...
def gen_frames():
    ...
    while True:
        ...
        target_angle, rate, difference, smoothed_avoidance = vision_steering.update(
            left_pixels, 
            right_pixels,
            avoidance_bias
        )
        ...
        # Display avoidance info
        if abs(smoothed_avoidance) > 0.1:
            direction = "RIGHT" if smoothed_avoidance > 0 else "LEFT"
            avoid_color = (0, 0, 255) if smoothed_avoidance > 0 else (0, 255, 0)
            cv2.putText(frame, f"AVOID {direction}: {abs(smoothed_avoidance):.2f}°/f",
                        (10, info_y + line_height * 4), cv2.FONT_HERSHEY_SIMPLEX,
                        0.55, avoid_color, 2)
...
<title>PiVision - Obstacle Avoidance Steering</title>
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-gradient-to-br from-orange-900 to-gray-800 p-5 rounded-lg border-2 border-orange-700">
                    ...
                    <div class="space-y-4 text-sm">
                        <div>
                            <div class="flex justify-between mb-1">
                                <span class="text-gray-400">Weight (°/pixel)</span>
                                <span class="text-white font-mono" id="avoid-weight-value">0.003</span>
                            </div>
                            <input type="range" id="avoid-weight-slider" min="1" max="20" value="3" 
                                   class="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-orange-500"
                                   oninput="updateAvoidWeightDisplay(this.value)" onchange="updateAvoidWeight(this.value)">
                            <div class="text-xs text-gray-500 mt-1">Strength of avoidance response</div>
                        </div>
                        
                        <div>
                            <div class="flex justify-between mb-1">
                                <span class="text-gray-400">Threshold (pixels)</span>
                                <span class="text-white font-mono" id="avoid-thresh-value">100</span>
                            </div>
                            <input type="range" id="avoid-thresh-slider" min="10" max="500" value="100" 
                                   class="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-orange-500"
                ...
                <div class="bg-gray-800 p-5 rounded-lg border border-gray-700">
                    ...
                    <div class="space-y-4 text-sm">
                        <div>
                            ...
                            <div class="text-xs text-gray-500 mt-1">Wall-following speed</div>
                        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-gradient-to-br from-cyan-900 to-gray-800 p-5 rounded-lg border border-cyan-700">
                ...
                <div class="text-sm text-gray-300 space-y-2">
                    ...
                    <p>• More left → turn right</p>
                    <p>• More right → turn left</p>
                </div>
            </div>
...
function updateAvoidWeightDisplay(value) {
    const weight = value / 1000;
    document.getElementById('avoid-weight-value').textContent = weight.toFixed(3);
}

function updateAvoidThreshDisplay(value) {
    document.getElementById('avoid-thresh-value').textContent = value;
}
...
function updateAvoidWeight(value) {
    const weight = value / 1000;
    fetch('/update_param', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({param: 'avoidance_weight', value: weight})
    })
    .then(response => response.json())
    .then(data => {
        console.log('Updated avoidance_weight:', data);
        showNotification('Avoidance weight updated', 'info');
    });
}

function updateAvoidThresh(value) {
    fetch('/update_param', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({param: 'avoidance_threshold', value: parseInt(value)})
    })
    .then(response => response.json())
    .then(data => {
        console.log('Updated avoidance_threshold:', data);
        showNotification('Avoidance threshold updated', 'info');
    });
}
...
def update_param():
    global AVOIDANCE_WEIGHT, AVOIDANCE_THRESHOLD
    ...
    elif param == 'avoidance_weight':
        AVOIDANCE_WEIGHT = value
    elif param == 'avoidance_threshold':
        AVOIDANCE_THRESHOLD = value
...
if __name__ == "__main__":
    try:
        ...
        print(f"  • Vision active: {vision_steering.is_active}")
        print("\nAvoidance Parameters:")
        print(f"  • Weight: {AVOIDANCE_WEIGHT}°/pixel")
        print(f"  • Threshold: {AVOIDANCE_THRESHOLD} pixels")
        print(f"  • Max avoidance: {MAX_AVOIDANCE}°/frame")
        print(f"  • Smoothing window: 3 frames")
        print("\nPID Parameters:")
